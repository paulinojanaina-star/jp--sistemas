...
//   CHECK time_off_requests_type_check: CHECK ((type = ANY (ARRAY['FERIAS'::text, 'FOLGA'::text, 'ATESTADO'::text, 'ANIVERSARIO'::text])))
...
//   $function$
//
// FUNCTION clear_expiry_dates_on_zero_stock()
...
//   $function$
//
// FUNCTION notify_whatsapp_alert()
...
// Table: items
//   trigger_clear_expiry_dates: CREATE TRIGGER trigger_clear_expiry_dates AFTER UPDATE OF current_quantity ON public.items FOR EACH ROW EXECUTE FUNCTION clear_expiry_dates_on_zero_stock()
// Table: notifications
//   on_notification_created_whatsapp: CREATE TRIGGER on_notification_created_whatsapp AFTER INSERT ON public.notifications FOR EACH ROW EXECUTE FUNCTION notify_whatsapp_alert()
